...
function TradeRow({ price, qty, time, side, isNew }: TradeRowProps) {
  ...
  return (
    <div className={`grid grid-cols-3 gap-2 text-xs font-mono py-1.5 px-2 rounded ${flashBg} ${transition}`}>
      <span className={textColor}>{price.toFixed(2)}</span>
      <span className="text-right text-zinc-700 dark:text-zinc-300">{qty.toFixed(4)}</span>
      <span className="text-right text-zinc-500 dark:text-zinc-400">{timeStr}</span>
    </div>
  );
}
...
export default function RecentTrades({ symbol = "btcusdt", useMock = true }: { symbol?: string; useMock?: boolean }) {
  ...
  return (
    <div className="rounded-md border border-zinc-200 dark:border-zinc-800 p-4">
      ...
      <div className="h-96 overflow-y-auto space-y-px border border-zinc-200 dark:border-zinc-700 rounded-md p-1">
        {trades.length > 0 ? (
          trades.map((trade) => (
            <TradeRow
